...
int Pon (char nombre[20], int socket, char pasw[50], TListaConnectados *lista){// ponemos un jugador en la partida que nos indique
...
void ConsultaLogin(int Logeado,char username[100],char password[100])
...
void *AtenderCliente(void *socket)
{
	...
	while(terminar ==0)
	{
		...
		if (codigo == -1)
		{
			...
			ConsultaLogin(Logeado,Username,Password);
		...
		else if (codigo ==4)
		{
			...
			char conectados2[1000];
			DameConectados(&listaconectados,conectados2);
			printf("%s\n",conectados2);	
			//printf(res);
			pthread_mutex_unlock(&excluir);
		...
		else if (codigo ==11)
		{
			...
			/*			{*/
			/*				write(listaconectados.conectados[j].socket,conectados[1],strlen(conectados));*/
			/*			}*/
